            ...
            match key:
                case ("devClustID" | "Name"):
                    if(not isinstance(devClusterData[key], str)):
                        raise web_exception(400, "Device Cluster's \"" + key + "\" parameter must be a string")
                    match key:
                        case "devClustID":
                ...
                case "deviceID":
                    if(not all(isinstance(deviceID, str) for deviceID in devClusterData["deviceID"])):
                        raise web_exception(400, "Device Cluster's \"deviceID\" parameter must be a list of strings")
            ...
            if("deviceID" in self.devClusterData.keys()):
                for deviceID in self.deviceID:
                    if(not check_presence_inDB(DBPath, "Devices", "deviceID", deviceID)):
                        raise web_exception(400, "Device with ID \"" + deviceID + "\" does not exist in the database")
    ...
    def deleteFromDB(DBPath, params):
        try:
            ...
            if(not check_presence_inDB(DBPath, "DevClusters", "devClustID", params["devClustID"])):
                raise web_exception(400, "Device Cluster with ID \"" + params["devClustID"] + "\" does not exist in the database")
